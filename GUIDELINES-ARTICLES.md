# Directives Éditoriales & Contraintes d'Environnement — noteimmo.com

Ce document récapitule les directives d'environnement et de monétisation obligatoires pour toute création de nouvel article sur **noteimmo.com**.
...
| :--- | :--- |
| **Domaine** | `noteimmo.com` |
| **Tracker Analytics** | Matomo cookieless (`https://analytics.les4h.fr/`) |
| **Conteneur MTM** | `container_OBWwBLzL.js` (Site ID: `109`) |
| **Adserver Revive** | `https://ads.les4h.fr/` (Éditeur ID: `114`) |
...
```html
<script type="text/javascript" src="https://analytics.les4h.fr/js/container_OBWwBLzL.js" async defer></script>
```
...
<!-- 1. Encart Pub In-Content Obligatoire (Medium Rectangle 300x250 - Zone 685) -->
<div class="my-12 py-6 border-y border-stone-200/80 flex justify-center">
  <ReviveSlot slot="inContent" />
</div>

<!-- 2. Encart Pub Fin d'Article Obligatoire (Large Rectangle 336x280 - Zone 683) -->
...
| :--- | :--- | :---: | :--- |
| **`header`** | Leaderboard (728x90) | **684** | Haut de page / sous navigation |
| **`inContent`** | Medium Rectangle (300x250) | **685** | Milieu d'article contextuel |
| **`largeRectangle`** | Large Rectangle (336x280) | **683** | Fin d'article avant recommandations |
| **`mobileSticky`** | Mobile Banner (320x100) | **686** | Bandeau sticky bas d'écran mobile |
| **`halfPage`** | Half Page (300x600) | **682** | Sidebar desktop pour articles longs |
| **`skyscraper`** | Skyscraper (160x600) | **687** | Colonne latérale de listing / dossiers |

---

## 4. Checklist Avant Publication
- [ ] L'article utilise le Layout avec le tracker MTM actif (`OBWwBLzL`).
