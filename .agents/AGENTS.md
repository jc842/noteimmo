# Directives & Contraintes d'Environnement — noteimmo.com

Ce document consigne l'architecture technique, les contraintes d'environnement, les directives de sécurité et les règles éditoriales pour le site **noteimmo.com** (noteimmo) hébergé sur Cloudflare Pages.
...
| :--- | :--- |
| **Domaine Apex** | `noteimmo.com` |
| **URL Cloudflare Pages** | `https://noteimmo.pages.dev` |
| **Dépôt GitHub** | `jc842/noteimmo` (branche principale : `main`) |
...
| **Matomo Site ID** | `109` |
| **Matomo MTM Container** | `container_OBWwBLzL.js` (Container ID : `OBWwBLzL`) |
| **Serveur Publicitaire** | Revive Adserver (`https://ads.les4h.fr/`) |
| **Revive Affiliate ID** | `114` |
| **Zones Publicitaires** | Header (`684`), In-Content (`685`), Mobile Sticky (`686`), Large Rectangle (`683`), Half Page (`682`), Skyscraper (`687`) |
...
- Ce Layout injecte au build statique le composant `Matomo.astro` avec le conteneur MTM dédié : **`container_OBWwBLzL.js`**.
...
- Tout article doit obligatoirement comporter les 2 emplacements publicitaires intégrés dans le gabarit (`src/pages/blog/[...slug].astro`) :
  1. **Encart In-Content (300x250)** : `<ReviveSlot slot="inContent" />` positionné après les premiers paragraphes (Zone 685).
  2. **Encart Fin d'Article (336x280)** : `<ReviveSlot slot="largeRectangle" />` positionné avant les suggestions de maillage (Zone 683).
